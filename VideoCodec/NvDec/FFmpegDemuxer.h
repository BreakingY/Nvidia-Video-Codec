...
private:
    ...
    */
    FFmpegDemuxer(AVFormatContext *fmtc, int64_t timeScale = 1000 /*Hz*/) : fmtc(fmtc) {
        ...
        //printf("fmtc->iformat->long_name:%s\n",fmtc->iformat->long_name);//hls->Apple HTTP Live Streaming
        bMp4H264 = eVideoCodec == AV_CODEC_ID_H264 && (
                ...
                || !strcmp(fmtc->iformat->long_name, "Apple HTTP Live Streaming")//hls+fmp4
            );
...
public:
    ...
    int GetParam(int &fps,int &bitrate){
        fps = av_q2d(fmtc->streams[iVideoStream]->avg_frame_rate);
        bitrate=fmtc->streams[iVideoStream]->codecpar->bit_rate;
        return 0;
    }
};
